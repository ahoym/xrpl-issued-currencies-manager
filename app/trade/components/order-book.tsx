...
export function OrderBook({
  ...
  const depthSummary: DepthSummary = {
    bidVolume: bids.reduce((acc, b) => acc.plus(b.total), new BigNumber(0)).toFixed(),
    bidLevels: bids.length,
    askVolume: asks.reduce((acc, a) => acc.plus(a.amount), new BigNumber(0)).toFixed(),
  ...
  const bestAsk =
    visibleAsks.length > 0 ? visibleAsks[visibleAsks.length - 1].price : null;
    ...
    <div>
      ...
      {depthSummary.bidLevels + depthSummary.askLevels > 0 && (
        <p className="mt-1.5 text-[11px] text-zinc-400 dark:text-zinc-500">
          ...
          {formatCompact(parseFloat(depthSummary.bidVolume))} {quoteCurrency} depth
          <span className="mx-1.5 text-zinc-300 dark:text-zinc-600">|</span>
          {depthSummary.askLevels} asks ·{" "}
          {formatCompact(parseFloat(depthSummary.askVolume))} {baseCurrency} depth
        </p>
      )}
      ...
      <div className="mt-3">
        ...
        {visibleAsks.length === 0 ? (
          ...
          visibleAsks.map((a, i) => {
            const isOwn =
              accountAddress !== undefined && a.account === accountAddress;
            const clickable = !isOwn && onSelectOrder;
            const barPct = maxAmount.gt(0)
              ? a.amount.div(maxAmount).times(100).toNumber()
              : 0;
            return (
              <div
                key={`ask-${i}`}
        ...
        {visibleBids.length === 0 ? (
          ...
          visibleBids.map((b, i) => {
            const isOwn =
              accountAddress !== undefined && b.account === accountAddress;
            const clickable = !isOwn && onSelectOrder;
            const barPct = maxAmount.gt(0)
              ? b.amount.div(maxAmount).times(100).toNumber()
              : 0;
            return (
              <div
                key={`bid-${i}`}
