...
export function TransferModal({
  ...
  const [recipientMode, setRecipientMode] = useState<"known" | "other">(
    "known",
  );
  ...
  const isIssuedCurrency =
    selectedBalance !== null && selectedBalance.currency !== Assets.XRP;
  const isBurn =
    isIssuedCurrency &&
    !!destinationAddress &&
    destinationAddress === selectedBalance?.issuer;
  const trustLineBlocked = isIssuedCurrency && trustLineOk === false;
  const ripplingBlocked =
    isIssuedCurrency && trustLineOk === true && ripplingOk === false;
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={
                selectedBalance ? `Max: ${selectedBalance.value}` : "0.00"
              }
              className="mt-1 w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-800"
            />
            {amount !== "" && !amountValid && (
          ...
          {isIssuedCurrency &&
            ...
            (isBurn ? (
              <p className="rounded-md bg-amber-50 p-3 text-sm text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                Recipient is the issuer. This will burn{" "}
                {selectedBalance?.currency} and reduce the outstanding supply.
              </p>
            ) : checkingTrustLine ? (
              <p className="text-xs text-zinc-500">Checking trust line...</p>
            ) : trustLineOk === false ? (
              <p className="rounded-md bg-amber-50 p-3 text-sm text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                Recipient does not have a trust line for{" "}
                {selectedBalance?.currency}. Set one up on the Setup page first.
              </p>
            ) : trustLineOk === true ? (
              ripplingOk === false ? (
                <p className="rounded-md bg-amber-50 p-3 text-sm text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                  Trust line exists, but the issuer does not have rippling
                  enabled. Enable it on the Setup page to allow peer-to-peer
                  transfers.
                </p>
              ) : (
