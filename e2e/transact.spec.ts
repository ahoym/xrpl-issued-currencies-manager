...
test.describe.serial("Transact page", () => {
  ...
  test("open TransferModal", async () => {
    ...
    await expect(
      page.getByRole("heading", { name: "Send Currency", level: 2 }),
    ).toBeVisible();
  ...
  test("currency dropdown has TCOIN", async () => {
    ...
    await expect(page.getByText("Loading balances...")).not.toBeVisible({
      timeout: 15_000,
    });

    // At least one option in the currency select should contain "TCOIN"
    await expect(
      page.locator("form select option", { hasText: "TCOIN" }).first(),
    ).toBeAttached();
  ...
  test("recipient mode toggle", async () => {
    ...
    const knownWalletButton = page.getByRole("button", {
      name: "Known wallet",
    });
  ...
  test("trust line validation for unknown address", async () => {
    ...
    const tcoinOption = currencySelect
      .locator("option", { hasText: "TCOIN" })
      .first();
    ...
    await expect(page.getByText(/does not have a trust line/)).toBeVisible({
      timeout: 15_000,
    });
  ...
  test("send TCOIN to issuer", async () => {
    ...
    await expect(page.getByText(/This will burn TCOIN/)).toBeVisible({
      timeout: 10_000,
    });
