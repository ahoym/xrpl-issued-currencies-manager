...
test.describe.serial("Trade AMM", () => {
  ...
  test("AMM Create modal UI", async () => {
    ...
    await expect(
      page.getByRole("heading", { name: "Create AMM Pool", level: 2 }),
    ).toBeVisible();
  ...
  test("AMM Create full flow", async () => {
    ...
    await expect(page.getByText("AMM pool created successfully!")).toBeVisible({
      timeout: 60_000,
    });
  ...
  test("AMM Deposit modal tabs", async () => {
    ...
    await expect(
      page.getByRole("heading", { name: "Deposit into AMM Pool", level: 2 }),
    ).toBeVisible();

    await expect(
      page.getByRole("button", { name: "Both Assets" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Single Asset" }),
    ).toBeVisible();
  ...
  test("AMM Withdraw modal tabs", async () => {
    ...
    await expect(
      page.getByRole("heading", { name: "Withdraw from AMM Pool", level: 2 }),
    ).toBeVisible();

    await expect(
      page.getByRole("button", { name: "Withdraw All" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Both Assets" }),
    ).toBeVisible();
