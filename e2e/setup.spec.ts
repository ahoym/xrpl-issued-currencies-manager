...
test.describe.serial("Setup page", () => {
  ...
  test("heading visible", async () => {
    ...
    await expect(
      page.getByRole("heading", {
        name: "XRPL Issued Currencies Manager",
        level: 1,
      }),
    ).toBeVisible();
  });
  ...
  test("show/hide seed", async () => {
    await expect(
      page.getByRole("button", { name: "Show secret" }),
    ).toBeVisible();
    await page.getByRole("button", { name: "Show secret" }).click();
    await expect(
      page.getByRole("button", { name: "Hide secret" }),
    ).toBeVisible();
  ...
  test("rippling already enabled", async () => {
    const ripplingButton = page.getByRole("button", {
      name: "Rippling Enabled",
    });
  ...
  test("export → clear → import roundtrip", async () => {
    ...
    await expect(
      page.getByRole("link", { name: /^r[a-zA-Z0-9]{24,}/ }).first(),
