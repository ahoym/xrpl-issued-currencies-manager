...
test.describe.serial("Compliance page", () => {
  ...
  test("generate Credential Issuer wallet", async () => {
    ...
    const credIssuerCard = page
      .locator("div")
      .filter({ has: page.getByText("Credential Issuer") })
      .first();
    const addressLink = credIssuerCard.getByRole("link", {
      name: /^r[a-zA-Z0-9]{24,}/,
    });
  ...
  test("issue KYC credential", async () => {
    ...
    await page.goto("/compliance");
    await expect(
      page.getByRole("heading", { name: "Issue Credential", level: 3 }),
    ).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("Issued Credentials table — Pending status", async () => {
    await page.goto("/compliance");
    await expect(
      page.getByRole("heading", { name: "Issued Credentials", level: 3 }),
    ).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("accept credential", async () => {
    ...
    await page.goto("/compliance");
    await expect(
      page.getByRole("heading", { name: "Recipient Credentials", level: 3 }),
    ).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("Create Domain form visible", async () => {
    ...
    await expect(
      page.getByPlaceholder("Credential issuer address"),
    ).toBeVisible();
  ...
  test("edit domain — form pre-fills, cancel returns", async () => {
    ...
    await expect(
      page.getByRole("heading", { name: "Edit Domain", level: 3 }),
    ).toBeVisible();
